"""Un script simple para ver todos los threads y mensajes guardados en SQLite, con formato usando Rich."""
...
parser = argparse.ArgumentParser(description="Ver threads y mensajes en la base de datos SQLite del historial de chat.")
parser.add_argument("--db", default=DB_PATH, help="Ruta a la base de datos SQLite (por defecto: chat_history.db)")
parser.add_argument("--values", action="store_true", help="Mostrar mensajes de cada thread (por defecto: solo listar threads)")
...
# Obtener todos los threads con conteo de mensajes
threads = conn.execute(
    "SELECT thread_id, COUNT(*) as count FROM messages GROUP BY thread_id ORDER BY thread_id"
).fetchall()

if not threads:
    print("[dim]No se encontraron threads en la base de datos.[/dim]")
    sys.exit(0)

print(f"\n[bold]Se encontraron {len(threads)} thread(s) en {args.db}[/bold]\n")
